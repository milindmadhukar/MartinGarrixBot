...
func main() {
	...
	for _, row := range rows {
		...
		if id == "" {
			unresolvable++
			slog.Debug("no apple id to resolve from",
		...
		if result == nil {
			notFound++
			slog.Warn("apple knows nothing about this id",
				slog.Int64("song_id", row.ID), slog.String("name", row.Name), slog.String("apple_id", id))
			continue
		}
...
func mergeInto(ctx context.Context, env *script.Env, row db.GetSongsWithPlaceholderDateRow, date string) bool {
	twin, err := env.Queries.GetSong(ctx, db.GetSongParams{
		Name: row.Name, Artists: row.Artists, ReleaseDate: date,
	})
	if err != nil {
