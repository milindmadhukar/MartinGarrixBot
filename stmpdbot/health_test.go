...
func callHealth(t *testing.T, b *STMPDBot) (*httptest.ResponseRecorder, HealthResponse) {
	t.Helper()
...
func TestHandleHealth_UnhealthyWithoutADatabase(t *testing.T) {
	...
	rec, resp := callHealth(t, &STMPDBot{
		IsReady: true,
		Version: "1.2.3",
		Commit:  "abcdef",
	})
...
func TestHandleHealth_ReportsTheGatewayState(t *testing.T) {
	...
	t.Run("ready", func(t *testing.T) {
		...
		_, resp := callHealth(t, &STMPDBot{IsReady: true})
	...
	t.Run("not ready", func(t *testing.T) {
		...
		rec, resp := callHealth(t, &STMPDBot{IsReady: false})
		if resp.Checks["discord"].OK {
