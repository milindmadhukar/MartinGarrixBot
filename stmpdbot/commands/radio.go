...
func RadioHandler(b *stmpdbot.STMPDBot) handler.CommandHandler {
...
func handleRadioStart(b *stmpdbot.STMPDBot, e *handler.CommandEvent) error {
...
func handleRadioStop(b *stmpdbot.STMPDBot, e *handler.CommandEvent) error {
	// Defer the response
	if err := e.DeferCreateMessage(false); err != nil {
		return err
	}
...
func handleRadioNowPlaying(b *stmpdbot.STMPDBot, e *handler.CommandEvent) error {
	guildID := *e.GuildID()
...
func handleRadioSkip(b *stmpdbot.STMPDBot, e *handler.CommandEvent) error {
	guildID := *e.GuildID()
	userID := e.User().ID
