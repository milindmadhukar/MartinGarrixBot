...
func buildSummaryEmbed(rows []db.GetSongAnniversariesRow, today time.Time) discord.Embed {
	...
	embed := discord.NewEmbedBuilder().
		SetTitle(fmt.Sprintf("On This Day - %s", today.Format("2 January"))).
		SetDescription(strings.Join(lines, "\n")).
		SetColor(utils.ColorSuccess).
		SetFooterText(fmt.Sprintf("%s in Martin Garrix history", pluralSongs(len(rows))))
	...
	for _, row := range rows {
		if row.Song.ThumbnailUrl.Valid && row.Song.ThumbnailUrl.String != "" {
			embed.SetThumbnail(row.Song.ThumbnailUrl.String)
			break
		}
	}
...
func buildSpotlightEmbed(row db.GetSongAnniversariesRow) (discord.Embed, bool) {
	...
	embed := discord.NewEmbedBuilder().
		SetAuthorName(spotlightHeading(row.YearsOld)).
		SetTitle(fmt.Sprintf("%s - %s", row.Song.Artists, row.Song.Name)).
		SetDescription(fmt.Sprintf("Released <t:%d:D> - <t:%d:R>", ts, ts)).
		SetColor(utils.ColorSuccess)

	if row.Song.ThumbnailUrl.Valid && row.Song.ThumbnailUrl.String != "" {
		embed.SetImage(row.Song.ThumbnailUrl.String)
	}

	return embed.Build(), true
}
...
func sendAnniversaries(b *mgbot.MartinGarrixBot, guild db.GetAnniversaryGuildsRow, rows []db.GetSongAnniversariesRow, today time.Time) {
	...
	for _, row := range rows {
		...
		// linkless case needs no guard here.
		for _, component := range utils.GetSongButtonRows(row.Song) {
			builder.AddContainerComponents(component)
		}

		if _, err := b.Client.Rest().CreateMessage(channelID, builder.Build()); err != nil {
